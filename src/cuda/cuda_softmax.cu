...
void SoftmaxWithTemperature(cudaStream_t stream, float* output, const float* input, int softmax_elements,
  ...
  constexpr int ILP = 1;
  dim3 block = SoftmaxGetBlockSize(ILP, softmax_elements);
  SoftmaxBlockForwardWithTemperature<ILP, float, float, float><<<grid, block, block.x * sizeof(float), stream>>>(
      output, const_cast<float*>(input), softmax_elements, input_stride, output_stride, temperature);
  CUDA_CHECK(cudaGetLastError());
}
