...
__global__ void AdvancedBlockReduceTopK(const float* __restrict__ scores_in, const int* __restrict__ indices_in,
  ...
  union SharedStorage {
    ...
    CubTempStorage cub_radix_storage;
  ...
  auto dispatch_kernels = [&](auto block_size_const) {
    ...
    switch (step.partitions_per_block) {
      case 16:
        ...
        else if (step.algorithm == ReductionAlgorithm::WARP_MERGE_SORT)
          LAUNCH_KERNEL(B_SIZE, 16, ReductionAlgorithm::WARP_MERGE_SORT);
        else
          LAUNCH_KERNEL(B_SIZE, 16, ReductionAlgorithm::WARP_BITONIC);
        break;
      case 8:
        if (step.algorithm == ReductionAlgorithm::CUB_RADIX_SORT)
          ...
          LAUNCH_KERNEL(B_SIZE, 8, ReductionAlgorithm::WARP_BITONIC);
        break;
      case 4:
        if (step.algorithm == ReductionAlgorithm::CUB_RADIX_SORT)
      ...
      case 2:
        if (step.algorithm == ReductionAlgorithm::CUB_RADIX_SORT)
          LAUNCH_KERNEL(B_SIZE, 2, ReductionAlgorithm::CUB_RADIX_SORT);
        else if (step.algorithm == ReductionAlgorithm::WARP_MERGE_SORT)
