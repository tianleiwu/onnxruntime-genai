 ...
 * The caller shall call __syncthreads() before and after this function.
 */
template <int kBlockSize, int SortSize>
...
__device__ inline void WarpBitonicSort(float& score, int& index) {
  ...
  for (int k = 2; k <= warpSize; k <<= 1) {
    for (int j = k >> 1; j > 0; j >>= 1) {
      ...
      // For a descending sort, we swap if the greater element is NOT in the correct
      // position according to the bitonic sequence direction.
      if (is_mine_greater != direction) {
        score = paired_score;
        index = paired_index;
      }
    }
...
__device__ void WarpMergeSort_cub(float* smem_scores, int* smem_indices, void* temp_storage_ptr, int num_valid_items) {
