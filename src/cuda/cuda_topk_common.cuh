 ...
 * It now dispatches to either Merge Sort or Radix Sort based on the partition size.
 */
...
__device__ void BlockReduceTopK(const float* scores_in_batch,
    ...
    if constexpr (kSortSize <= 32) {
    ...
    } else if constexpr (kSortSize <= 128) {
        // Load to shared memory and use CUB warp merge sort
        for (int i = threadIdx.x; i < kSortSize; i += kBlockSize) {
            if (i < num_elements_to_sort) {
                int part_idx = i / K_PADDED;
                int element_idx = i % K_PADDED;
        ...
        __syncthreads();
        bitonic_sort::WarpMergeSort<kSortSize>(smem.stage2_storage.scores, smem.stage2_storage.indices, &smem.cub_warp_storage, num_elements_to_sort);
    } else {
        // Load directly to registers and use CUB block merge sort
#ifdef STABLE_TOPK
        using SortKeyT = uint64_t;
    ...
    // Final write to global memory for warp-based sorts
    if (threadIdx.x < K_PADDED) {
