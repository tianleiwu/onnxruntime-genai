...
struct DeviceInterface {
  ...
  // Compute teacher-forcing log-probs on-device: out[i] = log_softmax(logits[i])[targets[i]].
  virtual void LaunchTargetLogProbs(const void* /*logits*/, ONNXTensorElementDataType /*logits_type*/, const int32_t* /*targets*/, float* /*out*/, int /*rows*/, int /*vocab_size*/) { assert(false); }
