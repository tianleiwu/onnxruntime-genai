...
class Model:
    ...
    def prepack_matmulnbits_weights(self, model_proto):
        ...
        by the target layout (SM80 -> {32, 64, 128}, SM90 -> {32, 64, 128}), K % block_size == 0,
        ...
        An offline-prepacked model runs on the fpA_intB kernel automatically: the
        `weight_prepacked` attribute forces the path, so ORT_FPA_INTB_GEMM is not required.
        """
        ...
        # The native SM90 fpA_intB kernel now serves block_size=32 via the multi-scale-per-K-tile
        # (ScaleKPerTile=2) path, so both the SM80 and SM90 layouts accept {32, 64, 128}. The weight
        # prepack byte-layout is block_size-independent (block_size only groups scales at runtime).
        allowed_block_sizes = (32, 64, 128)
