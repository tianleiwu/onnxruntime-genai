...
void PrintSummary(const std::vector<BenchmarkResult>& results) {
  std::cout << "\n--- Benchmark Summary ---\n";
  std::cout << std::left << std::setw(12) << "Batch Size" << std::setw(12)
            << "Vocab Size" << std::setw(5) << "K" << std::setw(28)
            << "Algorithm" << std::setw(12) << "Latency(us)" << std::setw(12)
            << "Stdev(us)" << std::setw(12) << "P95(us)" << "\n";
  ...
  for (const auto& result : results) {
    ...
    std::cout << std::left << std::setw(12) << result.params.batch_size
              << std::setw(12) << result.params.vocab_size << std::setw(5)
              << result.params.k << std::setw(28) << full_algo_name
              << std::fixed << std::setprecision(2) << std::setw(12)
              << result.latency_ms * 1000.0f << std::setw(12)
              << result.latency_ms_stdev * 1000.0f << std::setw(12)
              << result.latency_ms_95_percentile * 1000.0f << "\n";
  }
}

void RunBenchmarks(const BenchmarkParams& params, float temperature) {
  std::cout << "\n--- Running Benchmarks with batch_size=" << params.batch_size
  ...
  auto scores_in_d = Generators::CudaMallocArray<float>(
      static_cast<size_t>(params.batch_size) * params.vocab_size);
  auto scores_out_d = Generators::CudaMallocArray<float>(
      static_cast<size_t>(params.batch_size) * params.k);
  auto indices_out_d = Generators::CudaMallocArray<int>(
      static_cast<size_t>(params.batch_size) * params.k);
  ...
  auto bench_algo = [&](auto func) {
    ...
    return std::make_tuple(static_cast<float>(mean(latencies)),
                           static_cast<float>(stdev(latencies)),
  ...
  // Benchmark Full Sort
  {
    auto [mean_ms, stdev_ms, p95_ms] = bench_algo([&]() {
      Generators::cuda::RunTopKViaFullSort(
          data.get(), stream, scores_in_d.get(), scores_out_d.get(),
  ...
  if (params.k <= 64) {
    ...
    {
      auto scores_in_copy_d = Generators::CudaMallocArray<float>(
          static_cast<size_t>(params.batch_size) * params.vocab_size);
      // Selection sort will change input scores inplace.
      // Note that we exclude the copy latency to reflect the actual usage in sampling.
      cudaMemcpyAsync(scores_in_copy_d.get(), scores_in_d.get(),
                        sizeof(float) * params.batch_size * params.vocab_size,
                        cudaMemcpyDeviceToDevice, stream);
      auto [mean_ms, stdev_ms, p95_ms] = bench_algo([&]() {
        Generators::cuda::RunTopKViaSelectionSort(
            data.get(), stream, scores_in_copy_d.get(), scores_out_d.get(),
            indices_out_d.get(), params.vocab_size, params.batch_size, params.k,
            temperature);
      });
      all_results.push_back(
          {params, "SELECTION_SORT", 0, mean_ms, stdev_ms, p95_ms});
    }
    ...
    for (int p_size : {1024, 2048, 4096, 8192}) {
      ...
      auto [mean_ms, stdev_ms, p95_ms] = bench_algo([&]() {
        Generators::cuda::RunTopKViaHybridSort(
            data.get(), stream, scores_in_d.get(), scores_out_d.get(),
            indices_out_d.get(), params.vocab_size, params.batch_size, params.k,
            temperature, p_size);
      });
      all_results.push_back(
          {params, "HYBRID_SORT", p_size, mean_ms, stdev_ms, p95_ms});
    }
  }
