  ...
  if (params.k <= 64) {
    ...
    for (int partition_size : {1024, 2048, 4096, 8192}) {
        ...
        test_algo(algo_name, [&](float* s_d, int* i_d) {
          Generators::cuda::RunTopKViaHybridSort(sampling_data.get(), stream, scores_in_d.get(), s_d, i_d, params.vocab_size, params.batch_size, params.k, temperature, partition_size);
        });
    }

    for (int partition_size : {256, 512, 1024, 2048, 4096}) {
        if (partition_size > params.vocab_size) {
          continue;
        }
        std::string algo_name = "BITONIC (" + std::to_string(partition_size) + ")";
        test_algo(algo_name, [&](float* s_d, int* i_d) {
          Generators::cuda::RunTopKViaBitonicSort(sampling_data.get(), stream, scores_in_d.get(), s_d, i_d, params.vocab_size, params.batch_size, params.k, temperature, partition_size);
        });
    }
  }
...
void PrintSummary(const std::vector<BenchmarkResult>& all_results, const char* device_name) {
  ...
  struct SummaryData {
    ...
    // The following fields are used to validate classification rules
    float latency_hybrid_sort_partion_size_1024 = -1.0f;
    float latency_hybrid_sort_partion_size_2048 = -1.0f;
    float latency_hybrid_sort_partion_size_4096 = -1.0f;
  };
  ...
  for (const auto& result : all_results) {
    ...
    if (result.algo_name == "FULL_SORT") {
      ...
      if (result.partition_size == 1024) {
        summary_map[key].latency_hybrid_sort_partion_size_1024 = result.latency_ms;
      } else if (result.partition_size == 2048) {
        summary_map[key].latency_hybrid_sort_partion_size_2048 = result.latency_ms;
      } else if (result.partition_size == 4096) {
        summary_map[key].latency_hybrid_sort_partion_size_4096 = result.latency_ms;
      }
    } else {
  ...
  std::cout << "\n--- Summary of TopK Benchmark on " << device_name << " ---\n";
  std::cout << "batch_size,vocab_size,k,full_sort,selection_sort,bitonic_sort,hybrid_sort,hybrid_1024,hybrid_2048,hybrid_4096,selection/bitonic,hybrid/bitonic,best\n";
  ...
  for (const auto& pair : summary_map) {
    ...
    std::cout << std::get<0>(key) << ","
              ...
              << (data.latency_hybrid_sort_partion_size_1024 > 0 ? format_latency_ms_to_us(data.latency_hybrid_sort_partion_size_1024) : "N/A") << ","
              << (data.latency_hybrid_sort_partion_size_2048 > 0 ? format_latency_ms_to_us(data.latency_hybrid_sort_partion_size_2048) : "N/A") << ","
              << (data.latency_hybrid_sort_partion_size_4096 > 0 ? format_latency_ms_to_us(data.latency_hybrid_sort_partion_size_4096) : "N/A") << ","
...
void RunBenchmarks() {
  ...
  std::vector<int> vocab_sizes = {201088, 102400, 32000};  // GPT-OSS 201088, LLAMA2 32000, LLAMA3 128256, DeepSeek 102400, QWen3 1516465
  ...
  if constexpr (comprehensive) {
    vocab_sizes = {1 * 1024, 2 * 1024, 4 * 1024, 8 * 1024, 16 * 1024, 32 * 1024, 64 * 1024, 128 * 1024, 256 * 1024};
    for (int v = 32 * 1024; v <= 256 * 1024; v += 32 * 1024) {
      vocab_sizes.push_back(v);
    }

    batch_sizes = {1, 2, 4, 8};
    ks = {1, 2, 4, 6, 8, 12, 16, 32, 64}; // Threshold is in the range of 2 ~ 16, so we add more points in the range.
  }
  ...
  for (const auto& params : configs) {
    ...
    auto measure_latency = [&](const std::string& name, std::vector<double>& latency_list, int partition_size, auto func) {
      // Warmup
      ...
      auto average_latency = static_cast<float>(mean(latency_list));
      all_results.push_back({params, name, partition_size, 
                             average_latency,
    ...
    if (params.k <= 64) {
      measure_latency("SELECTION_SORT", latency_list, 0, [&]() {
      ...
      std::vector<int> parition_sizes = {256, 512, 1024};
      if (params.vocab_size >= 128 * 1024) {
        // In our tests, 2048 and 4096 is worse in our test. Add a few tests to confirm this.
        parition_sizes.push_back(2048);
        parition_sizes.push_back(4096);
      }
      for (int partition_size : parition_sizes) {
        if (partition_size > params.vocab_size)
          continue;
        std::string name = "BITONIC (" + std::to_string(partition_size) + ")";
        auto latency = measure_latency(name, latency_list, partition_size, [&, partition_size]() {
          Generators::cuda::RunTopKViaBitonicSort(sampling_data.get(), stream, scores_in.get(), scores_out.get(), indices_out.get(), params.vocab_size, params.batch_size, params.k, temperature, partition_size);
        });
      ...
      parition_sizes = {1024, 2048, 4096};
      // In our tests, 8192 is worse than 4096 when vocab_size <= 256 * 1024. Add a few tests to confirm this.
      if (params.vocab_size >= 128 * 1024) {
        parition_sizes.push_back(8192);
      }
      // In our tests, 512 is worse than 1024 when vocab_size >= 1024. Add a few tests to confirm this.
      if (params.vocab_size < 4 * 1024) {
        parition_sizes.push_back(512);
      }

      for (int partition_size : parition_sizes) {
        if (partition_size > params.vocab_size)
          continue;
        std::string name = "HYBRID (" + std::to_string(partition_size) + ")"; 
        auto latency = measure_latency(name, latency_list, partition_size, [&, partition_size]() {
          Generators::cuda::RunTopKViaHybridSort(sampling_data.get(), stream, scores_in.get(), scores_out.get(), indices_out.get(), params.vocab_size, params.batch_size, params.k, temperature, partition_size);
        });
        if (latency < best_latency) {
          best_latency = latency;
          data_b = latency_list;
      ...
      if (!comprehensive && params.batch_size == 1 &&  params.k == 50) {
        compare_statistics(data_a, data_b, name_a, name_b);
      }
    }

    measure_latency("FULL_SORT", latency_list, 0, [&]() {
  ...
  std::cout << "\n--- Benchmark Results on " << prop.name << " ---\n";
