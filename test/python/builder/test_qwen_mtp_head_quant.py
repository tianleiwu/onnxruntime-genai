...
def test_asymmetric_head_uses_unsigned_weight_dtype():
    model = _resolve(
        {"enable_mtp": True, "mtp_head_quant_type": "int8", "int4_is_symmetric": False}
    )
...
def test_float_main_model_gets_int4_defaults_for_a_fp4_head():
    model = _resolve(
        {"enable_mtp": True, "mtp_head_quant_type": "nvfp4"}, main_onnx_dtype=ir.DataType.FLOAT16
    )
...
def test_int4_lmhead_override_is_applied_as_mixed_precision():
    model = _resolve(
        {"enable_mtp": True, "mtp_head_quant_type": "int8", "mtp_head_int4_lmhead": "true"}
    )
