...
TEST(SamplingTests, RandomizedSamplingTopPAndKCuda) {
  ...
  params->SetSearchOption("top_k", k);
  params->SetSearchOption("top_p", p);
  params->SetSearchOption("temperature", 1.0f);
  params->SetSearchOption("batch_size", batch_size);

  std::random_device rd;
  std::mt19937 engine(rd());
  // random seed is int32 internally. Make sure it is in proper range.
  ...
  for (int i = 0; i < num_iter; i++) {
    int random_seed = dist(engine);
    params->SetSearchOption("random_seed", double(random_seed));
    auto generator = OgaGenerator::Create(*model, *params);

    // Set the same logits for every iteration to test sampling logic in isolation.
  ...
  // --- CPU-Side Expected Distribution Calculation (Correct) ---
  std::vector<float> top_k_logits(k);
  std::iota(top_k_logits.rbegin(), top_k_logits.rend(), 1.0f);  // {k, k-1, ..., 1}
  ...
  // --- Verification Loop (Fixed) ---
  // Check that the distribution of tokens generated by the model is close to the expected distribution
  for (auto& [logit, count] : logit_to_count) {
    // The logit values are k, k-1, ..., 1.
    // The expected_distributions vector is indexed from 0 to k-1, where
    // index 0 corresponds to logit k, index 1 to logit k-1, and so on.
    // The correct mapping from logit value to index is `k - logit`.
    const int logit_index = k - static_cast<int>(logit);

    // Ensure the calculated index is valid before using it.
    ASSERT_GE(logit_index, 0);
    ASSERT_LT(logit_index, k);

    const float expected_distribution = expected_distributions[logit_index];
    EXPECT_NEAR(count / float(total_count), expected_distribution, 0.01);
  }
}
...
TEST(SamplingTests, RandomizedSamplingTopPAndKWithTemperatureCuda) {
  ...
  // --- CPU-Side Expected Distribution Calculation ---
  std::vector<float> top_k_logits(k);
  std::iota(top_k_logits.rbegin(), top_k_logits.rend(), 1.0f); // {k, k-1, ..., 1}
  ...
  std::vector<float> expected_distributions = filtered_logits;
  // --- Use temperature 1.0f for the final re-normalization step ---
  Softmax(expected_distributions, 1.0f);
